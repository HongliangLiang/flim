...
def main():
    ...
    #/data/hdj/tracking_buggy_files/joblib_memmap_tomcat/tomcat/data_memmap
...
def process(ptemplate, fold_number, fold_testing, fold_training, file_prefix,project):
    ...
    :param ptemplate: 指模型
    :param fold_number: 有几份数据 2 下标从0开始
    :param fold_testing: 3个测试数据 里面是dataframe
    :param fold_training: 3的训练数据 dataframe
    :param file_prefix: swt/swt
    :return: {"name": ptemplate.name,模型名字
            "results": calculate_metric_results(all_results_df) 这里是top20 map mrr
            }
            [{'name': 'Adaptive', 'results': ({1: 0.3429355281207133, 2: 0.46227709190672156, 3: 0.5349794238683128, 4: 0.5914951989026063, 5: 0.6315500685871056, 6: 0.6595336076817558, 7: 0.6858710562414266, 8: 0.7058984910836763, 9: 0.725925925925926, 10: 0.7374485596707819, 11: 0.7489711934156379, 12: 0.7602194787379972, 13: 0.7720164609053498, 14: 0.7838134430727023, 15: 0.7906721536351166, 16: 0.798079561042524, 17: 0.8021947873799725, 18: 0.8079561042524005, 19: 0.8137174211248285, 20: 0.8200274348422496}, 0.40892458362741296, 0.47245062488563383, range(1, 21))}]

    '''
    results_list = []

    for i in range(fold_number):#8 0-7
        #开始使用每一fold
        #0训练 1测试
        #1训练 2测试
        ...
        if r is None:
            ...
            gc.collect()
            return None
        #找到r等于1的result的最小值，将r的result值>=min的值都复制一份
        #意思就是所有fixed文件里取分值最低的那个，然后将所有分值≥min的文件都保留，最终用作计算指标
        #这里是把所有fold的fix得分最小值计算出来进行过滤，所以针对每一个report来说还可以进一步过滤
    ...
    with open('/data/hdj/tracking_buggy_files/featureMerge/all_results_df_before_1017_lstm_'+project+'.pickle', 'wb') as file:
    ...
    with open('/data/hdj/tracking_buggy_files/featureMerge/all_results_df_after_1017_lstm_'+project+'.pickle', 'wb') as file:
        pickle.dump(all_results_df, file, protocol=pickle.HIGHEST_PROTOCOL)
